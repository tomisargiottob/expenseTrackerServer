...
class TransactionController {
        ...
        try {
          const transactions = await Transaction.find({
            ...(frequency !== "custom"
              ? {
                  date: {
                    $gt: subDays(new Date(), Number(frequency)).toDate()
                  },
                }
              : {
                  date: {
    ...
    static async updateTransaction (req, res) {
        try {
          await Transaction.findOneAndUpdate({_id : req.params.id, organization: req.params.organizationId} , req.body.payload)
